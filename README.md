...
- LCSC 속성이 없는 심볼은 EasyEDA/JLCPCB 정확 매칭으로 자동 채우고, 찾지 못하면 빈 `LCSC` 속성 추가
...
기존 라이브러리 심볼에 `LCSC` 속성이 없거나 값이 비어 있는 경우 `라이브러리 > 누락 LCSC 자동 채우기` 메뉴로 EasyEDA/JLCPCB 정확 매칭 검색을 실행할 수 있습니다. 찾지 못한 심볼은 빈 `LCSC` 속성으로 남습니다.
